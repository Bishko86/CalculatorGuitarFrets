...
function invalidData(selector) {
  ...
  let message =
    '<i class="fa fa-exclamation-triangle fa-lg"></i>' + "invalid data";
...
function addTable(scale, num, measure) {
  ...
  trHeader.innerHTML = "<th> Fret #</th> <th>nut offset</th> <th>prev.fret</th>";
...
function calcGuitarFrets(beaker, amount, constanta, measureUnit) {
  ...
  for (let i = 0; i < amount; i++) {
    ...
    let dataFret = `<span class="data"> ${roundNumber(
      fromZero
    )} ${unit}</span>`;
